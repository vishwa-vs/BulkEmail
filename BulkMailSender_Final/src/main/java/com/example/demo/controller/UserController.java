...
public class UserController {
    ...
    public String signin(UserEntity uEntity,HttpSession session) {
	
        return "signin";
	}
	
	@GetMapping("/new") // Storing the Sign In Details And Returns the Login Page
	//@PreAuthorize("hasAuthority('ROLE_USER')")
    public String index(UserEntity uEntity,Model model) {
		//uEntity.setUpass(passwordEncoder.encode(uEntity.getUpass()));
		
		boolean f = userService.checkEmail(uEntity.getUemail());
